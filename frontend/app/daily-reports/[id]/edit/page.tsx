...
import { dailyReportsApi, DailyReport, DailyReportCreate } from '@/lib/daily-reports'
...
export default function DailyReportEditPage({ params }: { params: { id: string } }) {
  ...
  const loadReport = async () => {
    try {
      setLoading(true)
      const report = await dailyReportsApi.getDailyReport(parseInt(params.id))
      setFormData(report)
    } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setSaving(true)
      await dailyReportsApi.updateDailyReport(parseInt(params.id), formData as DailyReportCreate)
      alert('日報を更新しました')
          ...
          <div className="space-y-6">
            ...
            <div className="flex gap-4 justify-end">
              <Button
                type="button"
                variant="outline"
                onClick={() => router.push(`/daily-reports/${params.id}`)}
                disabled={saving}
              >
                キャンセル
              </Button>
